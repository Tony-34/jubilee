     ...
     if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $email = test_input($_REQUEST["email"]);
       ...
       if(empty($email) || empty($phone)){
          echo "<script>swal('Failed', 'Please fill all the fields', '');</script>";
       } else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
          echo "<script>swal('Failed', 'Please enter the correct email', '');</script>";
       }else if (!(preg_match("/^[0-9]{12}$/", $phone))){
          echo "<script>swal('Failed', 'Please the correct phone number', '');</script>";
